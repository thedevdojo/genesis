                ...
                <div :class="{'block bg-white dark:bg-gray-900 relative z-50 w-full h-auto px-4 py-5 left-0 mt-16': open, 'hidden': ! open}" class="items-center space-y-3 sm:space-x-3 sm:space-y-0 sm:mt-0 sm:bg-transparent sm:p-0 sm:relative sm:flex sm:-my-px sm:ml-8" x-cloak>
                    @foreach($navLinks as $title => $route)
                        <a href="{{ route($route) }}"
                            ...
                            class="rounded-md px-4 sm:px-3 py-2.5 sm:py-2 sm:w-auto text-sm group relative @if(Request::is($route)){{ 'text-gray-800 bg-gray-100/70 dark:bg-gray-800/70 dark:text-gray-100 font-semibold' }}@else{{ 'text-gray-600 font-medium hover:bg-gray-100/70 dark:text-white/70 dark:hover:bg-gray-800/70 dark:hover:text-gray-100' }}@endif">
                            <span>{{ $title }}</span>
                        </a>
                    @endforeach
                </div>
                    ...
                    >
                        ...
                        <div x-show="dropdownOpen" @click.away="dropdownOpen=false" x-transition:enter="transition ease-out duration-100" x-transition:enter-start="transform opacity-0 sm:scale-95" x-transition:enter-end="transform opacity-100 sm:scale-100" x-transition:leave="transition ease-in duration-75" x-transition:leave-start="transform opacity-100 sm:scale-100" x-transition:leave-end="transform opacity-0 sm:scale-95" 
                            ...
                            <div class="p-4 pt-0 mt-1 space-y-3 text-gray-600 dark:text-white/70 bg-white dark:bg-gray-900 dark:shadow-xl sm:p-1 sm:space-y-0 sm:border sm:shadow-md sm:rounded-md border-gray-200/70 dark:border-white/10">
                                <a href="{{ route('profile.edit') }}" class="relative flex cursor-pointer hover:text-gray-700 dark:hover:text-white/70 select-none hover:bg-gray-100/70 dark:hover:bg-gray-800/80 items-center rounded py-2 px-4 sm:px-2 sm:py-1.5 text-sm outline-none transition-colors data-[disabled]:pointer-events-none data-[disabled]:opacity-50">
